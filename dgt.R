# Pulling in Ben's improvements (https://www.kaggle.com/users/993/ben-hamner/digit-recognizer/rf-proximity) 
...
train <- data.frame(fread("C:\\Users\\Varsha\\Desktop\\GirGit\\GitHubStrat\\input\\train.csv", header=TRUE))
...
# Do MDS on subsetof proximities and get the portion of the proximity matrix for holdout set:
...
p <- ggplot(data.frame(embeddingSmall$points), aes(x=X1, y=X2)) + 
  ...
  theme(strip.background = element_blank(),
        ...
        legend.position  = "top") +
  xlab("") + ylab("") + 
  ggtitle("MNIST Embedding Using Random Forest Proximity\n*red numbers are classification error*")
